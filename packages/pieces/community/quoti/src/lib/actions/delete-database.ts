...
export const deleteDatabaseItem = createAction({
  // auth: check https://www.activepieces.com/docs/developers/piece-reference/authentication,
  name: 'deleteDatabaseItem',
  auth: quotiAuth,
  displayName: 'Delete from database',
  description: 'Delete a specific item from a database',
  props: {
  ...
  async run(context) {
    ...
    const res = await axios.delete(`https://api.quoti.cloud/api/v1/${context.auth.org_slug}/resources/${context.propsValue['databaseSlug']}/${context.propsValue['id']}`, {
      headers: {
